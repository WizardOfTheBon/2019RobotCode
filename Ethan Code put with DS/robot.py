...
class MyRobot(wpilib.TimedRobot):
	def __init__(self):
		...
		self.pulleyEncoderJamThreshold = 200
		self.pulleyIdlingStrikes = 0
		self.pulleyIdlingStrikesThreshold = 6
		self.lastPosition = 0 # last position is used in the pulley height function to check the movement, and it's value here doesn't matter much
		...
		self.switchesChannel = 2
		self.buttonsChannel = 1
		self.driveJoystickChannel = 0
		
		self.ds = wpilib.DriverStation.getInstance()
		self.driveStick = wpilib.Joystick(self.driveJoystickChannel)
		self.buttonsStick = wpilib.Joystick(self.buttonsChannel)
		self.switchesStick = wpilib.Joystick(self.switchesChannel)
	...
	def pulleyHeight(self, level, payload): # level 0 is the floor, payload 1 is hatch, payload 2 is cargo, payload 3 is cargo ship cargo(not done yet)
		...
		if payload == 1: # hatch
			...
			goalPosition = self.ticksPerInchPulley * self.cargoShipHeightInches #this is the cargoship thing, ben likes to do the if if only left over thing
	...
	def teleopPeriodic(self):
		...
		if self.auxillary12.getRawButton(1) ==1:
			if self.IRsensor > 0.1 and self.IRsensor < self.Ball_dist and self.adjust_ready == 0 self.auxillary1.getRawButton(6) ==1: #this is supposedly the ball distance thing
			...
			if self.IRsensor > 0.1 and self.IRsensor < self.hatch_dist and self.adjust_ready == 0: #this is supposedly the ball distance thing
				self.pulleyMotorModifier = 0.05
				self.pulleyMotor.set(self.pulleyMotorModifier)
				if self.pulleyMotor.getQuadraturePosition() > 10:
					self.pulleyMotorModifier += 0.05
			...
			if self.IRsensor > 0.1 and self.IRsensor < self.ball_dist and self.adjust_ready == 0 and self.auxillary1.getRawButton(7) ==1: #this is supposedly the ball distance thing
				self.pulleyMotorModifier = 0.05
				self.pulleyMotor.set(self.pulleyMotorModifier)
				if self.pulleyMotor.getQuadraturePosition() > 10:
					self.pulleyMotorModifier += 0.05
