...
class MyRobot(wpilib.TimedRobot):
	def __init__(self):
		...
		self.pulleyMotorModifier = 0.4 # slows down the Pulley motor speed just in case it goes way too fast
	...
	def checkSwitches(self):
		...
		if self.auxiliary1.getRawButton(self.eStop): #E-Stop button pressed, stop all motors and remove all jobs from job queue.
			...
			if self.auxiliary2.getRawButton(self.rightLeadScrewDown): # right lead screw out manual
				...
				self.rightLeadScrewMotor.set(0)
				print(str(self.rightLeadScrewMotor.getQuadraturePosition()))
	...
	def teleopPeriodic(self):
		...
		self.drive.driveCartesian(self.driveStick.getX(), self.driveStick.getY() *-.8, self.driveStick.getZ()*.9, 0)
		
		print(str(self.rightLeadScrewMotor.get()))
		...
		'''if self.auxiliary2.getRawButton(1) ==1:
			if (self.IRSensor.getVoltage() > 0.1) and (self.IRSensor.getVoltage() < self.ball_dist) and (self.adjust_ready == 0) and (self.auxiliary1.getRawButton(6) ==1): #this is supposedly the ball distance thing
			...
			if (self.IRSensor.getVoltage() > 0.1) and (self.IRSensor.getVoltage() < self.hatch_dist) and (self.adjust_ready == 0): #this is supposedly the ball distance thing
				self.pulleyMotorModifier = 0.05
				self.pulleyMotor.set(self.pulleyMotorModifier)
				if self.pulleyMotor.getQuadraturePosition() > 10:
					self.pulleyMotorModifier += 0.05
			...
			if (self.IRSensor.getVoltage() > 0.1) and (self.IRSensor.getVoltage() < self.ball_dist) and (self.adjust_ready == 0) and (self.auxiliary1.getRawButton(7) ==1): #this is supposedly the ball distance thing
				self.pulleyMotorModifier = 0.05
				self.pulleyMotor.set(self.pulleyMotorModifier)
				if self.pulleyMotor.getQuadraturePosition() > 10:
					self.pulleyMotorModifier += 0.05
